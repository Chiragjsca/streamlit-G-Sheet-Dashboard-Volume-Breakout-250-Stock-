...
# ── ULTRA HD CHART STYLING & RENDERING ─────────────────────
                        ...
                        # A single, continuously CHAINED flow — like a real financial Sankey —
                        # instead of three separate floating blocks: financing sources feed into
                        # Total Financing, which flows into Total Assets, which breaks into asset
                        # categories (incl. Trade Receivables) and flows onward into Net Sales,
                        # which finally splits into Net Profit / Total Expenses. Trade Payables is
                        # included as a financing source.
                        _mg_labels, _mg_colors = [], []
                        _mg_src, _mg_tgt, _mg_val, _mg_lcolor = [], [], [], []

                        def _mg_add(label, color):
                            _mg_labels.append(label)
                            _mg_colors.append(color)
                            return len(_mg_labels) - 1

                        _mg_prev_hub = None  # index of the last "total" hub, so each stage chains onto the previous one
                        ...
                        if _mg_has_financing:
                            ...
                            _mg_fin_idx = _mg_add(f"Total Financing<br>₹{_mg_fin_total:,.2f} Cr", "#5c6bc0")
                            for n, v, c in _mg_cs_valid:
                                idx = _mg_add(f"{n}<br>₹{v:,.2f} Cr", c)
                                _mg_src.append(_mg_fin_idx); _mg_tgt.append(idx); _mg_val.append(v)
                                _mg_lcolor.append(_hex2rgba(c))
                            _mg_prev_hub = _mg_fin_idx

                        # Stage 2 — Total Assets → asset categories (incl. Trade Receivables),
                        # chained onto Stage 1's hub so the diagram flows continuously left-to-right.
                        ...
                        if _mg_has_assets:
                            ...
                            _mg_assets_idx = _mg_add(f"Total Assets<br>₹{_mg_total_assets:,.2f} Cr", "#37474f")
                            if _mg_prev_hub is not None:
                                _mg_src.append(_mg_prev_hub); _mg_tgt.append(_mg_assets_idx); _mg_val.append(_mg_total_assets)
                            ...
                            for n, v, c in _mg_ad_nodes:
                                ...
                                _mg_lcolor.append(_hex2rgba(c))
                            _mg_prev_hub = _mg_assets_idx

                        # Stage 3 — Net Sales → Net Profit / Total Expenses, chained onto whichever
                        # hub came before it (Total Assets, or Total Financing if Assets is unavailable).
                        ...
                        if _mg_has_revenue:
                            ...
                            _mg_sales_idx = _mg_add(f"Net Sales<br>₹{_mg_sales:,.2f} Cr", "#1565C0")
                            if _mg_prev_hub is not None:
                                _mg_src.append(_mg_prev_hub); _mg_tgt.append(_mg_sales_idx); _mg_val.append(_mg_sales)
                                _mg_lcolor.append(_hex2rgba("#1565C0"))
                            _mg_profit_idx = _mg_add(f"Net Profit<br>₹{_mg_profit:,.2f} Cr", "#0f9d58")
                            _mg_exp_idx = _mg_add(f"Total Expenses<br>₹{_mg_exp:,.2f} Cr", "#ea4335")
                        ...
                        if _mg_groups_shown > 0:
                            ...
                            st.caption(
                                "All money-related flows chained into ONE continuous chart: financing sources "
                                "(Equity + Reserves + Debt + Trade Payables) → Total Financing → Total Assets → "
                                "asset categories (incl. Trade Receivables) → Net Sales → Net Profit / Total "
                                "Expenses. Total Financing, Total Assets and Net Sales come from different rows "
                                "in your sheet and don't reconcile to one exact number, so each bridge link uses "
                                "the receiving stage's own reported total — the same approximation real "
                                "financial Sankeys make when chaining balance-sheet and P&L figures together. "
                                "Trade Payables now also appears in the 🏗️ Capital Structure chart above."
                            )
                        else:
