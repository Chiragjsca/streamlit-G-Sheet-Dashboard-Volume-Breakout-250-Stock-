...
# ── ULTRA HD CHART STYLING & RENDERING ─────────────────────
                        ...
                        # Merges the three "money" diagrams above — Capital Structure (financing),
                        # Asset Deployment, and Revenue & Expenses — into ONE chart, as requested.
                        # Trade Payables (financing/liability side) and Trade Receivables (asset
                        # side) are both included here.
                        # The three groups are drawn side-by-side in a single figure rather than
                        # chained end-to-end (e.g. Total Financing → Total Assets → Net Sales),
                        # because those three totals come from different rows/sources in your sheet
                        # and don't reconcile to one consistent number — chaining them would draw a
                        # link your data doesn't actually support. Keeping them as separate flows in
                        # one chart is the honest way to merge them.
                        ...
                        # Group A — Financing (incl. Trade Payables)
                        _mg_cs_parts = [
                        ...
                        # Group B — Asset Deployment (incl. Trade Receivables)
                        ...
                        if _mg_groups_shown > 0:
                            ...
                            fig_merged.update_layout(
                                ...
                                template="plotly_white", height=280 + 140 * _mg_groups_shown,
                                margin=dict(t=45, b=10, l=10, r=10),
                                font=dict(size=12),
                            )
                            st.plotly_chart(fig_merged, use_container_width=True, key=f"sankey_merged_{sym}")
                            st.caption(
                                "All money-related flows for this stock, merged into one chart: financing "
                                "(Equity + Reserves + Debt + Trade Payables), asset deployment (incl. Trade "
                                "Receivables), and revenue → profit/expenses — shown side-by-side in a single "
                                "figure. They're not chained end-to-end because Total Financing, Total Assets "
                                "and Net Sales come from different rows in your sheet and don't reconcile to "
                                "one number; chaining them would fabricate a link your data doesn't support. "
