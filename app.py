...
KT='Automatically adjust column widths based on text length of the selected row.'
KS=' (100%)'
KR='Other Assets (unspecified)'
KQ='Cash & Equivalents'
KP='#00897b'
KO='Trade Receivables'
KN='Inventory'
KM='#5e35b1'
KL='Fixed Assets / Net PPE'
KK='#5c6bc0'
KJ='#8d6e63'
KI='Trade Payables'
KH='Total Debt'
...
Jy='Candle'
Jx='%d %b %Y %H:%M'
Jw='⚠️ No AI configured. Add `GEMINI_API_KEY` or `GROQ_API_KEY` to Streamlit secrets.'
Jv='stock name'
Ju='company name'
Jt='Type symbol name...'
Js='Search symbol:'
Jr='Stocks'
Jq='%{customdata}: %{y:.2f}%<extra></extra>'
Jp='displaylogo'
Jo='#e3f2fd'
Jn='close price'
Jm='%Y%m%d_%H%M'
...
Je='50 DMA < 200 DMA'
Jd='50 DMA > 200 DMA'
Jc='50 DMA > 100 DMA > 200 DMA'
Jb='50 DMA < 100 DMA < 200 DMA'
Ja='All (No Filter)'
JZ='macd crossover'
JY='start gtt order'
JX='output'
...
JU='#f4b400'
JT='bf_search'
JS='perf_matrix_search'
JR='main_matrix_search'
JQ='search_query'
JP='50 dma'
JO='d/e ratio'
JN='52w low'
...
JG='history data'
JF='trading view'
JE='1SFhuZbLLlwwFsNo1k2RRx_Zp6bAkRR20W0F_zTwgdwU'
...
from google.auth.transport.requests import AuthorizedSession as Ky
...
from st_aggrid.shared import GridUpdateMode as Kz
import streamlit.components.v1 as P,re,io,google.generativeai as Go,plotly.graph_objects as O
from plotly.subplots import make_subplots as K_
...
Cd=J
BN=J
if J7 in A.secrets:Go.configure(api_key=A.secrets[J7]);Cd=B
if J8 in A.secrets:
	try:from groq import Groq as L0;L1=L0(api_key=A.secrets[J8]);BN=B
	except ImportError:BN=J
EY=Cd or BN
def EZ(prompt,model_choice):
	A=prompt
	if model_choice==Fs and BN:B=L1.chat.completions.create(model='llama-3.3-70b-versatile',messages=[{'role':'user','content':A}],max_tokens=2048);return B.choices[0].message.content
...
L5={BD:['50 DMA','100 DMA','200 DMA','NSE 1','Trading View 1','History Data 1','Screener 1','Zerodha 1','Chartlink 1','Market smith india 1','Official NSE URL 1'],BE:[],CM:[],CN:[],CO:[],CP:[],CQ:[]}
L6={BD:['E','F','G','AA','AB','AC','AD','AE','AF','AG','AH'],BE:[],CM:[],CN:[],CO:[],CP:[],CQ:[]}
def Gp(letter):
	...
	return B-1
def L7(sheet_name,ordered_columns):
	...
	return B
L8={BD:D,BE:D,CM:D,CN:D,CO:D,CP:D,CQ:D}
L9={BD:[w,Aj,'Close Price',AM,J9,Ak,Al,JA,'Differance from 200 DMA','Cumulative Average Rule (CAR) Rating'],BE:[],CM:[],CN:[],CO:[],CP:[],CQ:[]}
LA={BD:['B','C','D','L'],BE:[],CM:[],CN:[],CO:[],CP:[],CQ:[]}
def LB(sheet_name,ordered_columns):
...
A.markdown(LC,unsafe_allow_html=B)
import streamlit as A
LD='\n<style>\n    [data-testid="stToolbar"] {\n        right: 2rem;\n    }\n    [data-testid="stToolbar"]::before {\n        content: "";\n    }\n    button[kind="header"] {display: none;}\n</style>\n'
A.markdown(LD,unsafe_allow_html=B)
LE='romo'
...
def DK(sheet_name):
	...
	try:
		...
		D=A.secrets[Dv]
		if Ay(D,G):D=EV.loads(D)
		Y=[JC,JD];N=Gn.from_service_account_info(D,scopes=Y);j=EU.authorize(N);Z=JE;a=urllib.parse.quote(sheet_name);b=Ky(N);c=f"https://sheets.googleapis.com/v4/spreadsheets/{Z}?includeGridData=true&ranges={a}";d=b.get(c);E=d.json()
...
def DL(df,col_name,st_container,display_label=D):
	...
	if D in df.columns:
		A=df[D].astype(G).str.replace(BG,C,regex=B);A=H.to_numeric(A,errors=AN).replace([A6.inf,-A6.inf],A6.nan);E=A.dropna()
		if not E.empty:
...
def By(val):
	...
	A=re.sub(Dw,C,G(val));return Q(A)
def Gu(df):
	A=df.copy();D=[A for A in A.columns if A.startswith(AX)or A.startswith(AY)or A==n];A=A.drop(columns=D,errors='ignore')
	for B in A.select_dtypes(include=['object']).columns:A[B]=A[B].apply(lambda x:re.sub(Dw,C,G(x))if H.notnull(x)else x)
	return A
...
with A.expander('🏆 Click to view Full-Market India Rankings',expanded=J):
	LP,LQ,LR,LS,LT=A.tabs(['🚀 Gainers & Losers','📦 Volume & Active','⭐ 52W High / Low','🔄 52W Reversals','📊 Top 100 Traded'])
	...
	with LT:A.markdown("<p style='font-size:14px; font-weight:bold;'>📊 Top 100+ Stocks Traded (Full India Screener)</p>",unsafe_allow_html=B);P.html(Aq('general'),height=520)
A.write(c)
@A.cache_data(ttl=300)
def LU():
	B=DK(BD);A={}
...
def LX():
	...
	F=[A for A in E.columns if not A.startswith(AX)and not A.startswith(AY)];I=U((A for A in F if A.lower()in[D0,AZ,Dx,Dy,D1,Dz]),D);J=U((A for A in F if A4 in A.lower()),D);K=U((A for A in F if D2 in A.lower()or y in A.lower()),D);L=U((A for A in F if BH in A.lower()),D);M=U((A for A in F if D_ in A.lower()and'face'not in A.lower()and'enterprise'not in A.lower()),D);N=U((A for A in F if CS in A.lower()),D)
	if not I:return H.DataFrame()
	A=H.DataFrame();A[j]=E[I].astype(G).str.strip();A[AM]=H.to_numeric(E[J].astype(G).str.replace(BG,C,regex=B),errors=AN)if J else i;A[E0]=H.to_numeric(E[K].astype(G).str.replace(BG,C,regex=B),errors=AN)if K else i;A[w]=H.to_numeric(E[L].astype(G).str.replace(BG,C,regex=B),errors=AN)if L else i;O=A[AM]*A[w]
	if M:A[CT]=H.to_numeric(E[M].astype(G).str.replace(P,C,regex=B),errors=AN)
	else:A[CT]=O
	if N:A[D3]=H.to_numeric(E[N].astype(G).str.replace(P,C,regex=B),errors=AN)
	else:A[D3]=O
...
def B3(dataframe,metric_label=y):
	...
	for(R,B)in J.iterrows():
		...
		elif G==JM:D=B.get(w,0);E=B.get(CT,0);O=f"{D/1000000:.1f}M"if D>=1000000 else f"{D/1000:.1f}k";P=f"₹{E/10000000:,.1f}Cr"if E>=10000000 else f"₹{E:,.0f}";F=f"📦 {O} | 💰 {P}"
		else:F=f"{N}{H:.2f}%"
...
with A.expander('🏆 Click to view Advanced Ranking Dashboards (Top 250 Stocks)',expanded=J):
	if Ae.empty:A.info("Ranking data is currently unavailable. Please check the 'Top 250 Stocks' sheet.")
		...
		LY=Ae.nlargest(20,E0);LZ=Ae.nsmallest(20,E0);La=Ae.nlargest(20,w);Lb=Ae[Ae[w]>0].nsmallest(20,w);Lc=Ae.nlargest(20,w);Ld=Ae.nlargest(20,CT);Le=Ae.nlargest(20,D3);Lf=Ae.nlargest(20,CT);Lg,Lh,Li,Lj,Lk,Ll=A.tabs(['📈 Gainers/Losers','📦 Volume Leaders','🔥 Active (Vol & Val)','💰 Top by Value','💎 Top by Turnover','💰 Most Active'])
		with Lg:A.markdown("<p style='font-size:14px; font-weight:bold; margin-top:10px;'>🚀 Top 20 Gainers</p>",unsafe_allow_html=B);A.markdown(B3(LY,y),unsafe_allow_html=B);A.markdown("<p style='font-size:14px; font-weight:bold; margin-top:10px;'>🔻 Top 20 Losers</p>",unsafe_allow_html=B);A.markdown(B3(LZ,y),unsafe_allow_html=B)
		with Lh:A.markdown("<p style='font-size:14px; font-weight:bold; margin-top:10px;'>📦 Top 20 by Volume</p>",unsafe_allow_html=B);A.markdown(B3(La,BH),unsafe_allow_html=B);A.markdown("<p style='font-size:14px; font-weight:bold; margin-top:10px;'>💤 Bottom 20 by Volume</p>",unsafe_allow_html=B);A.markdown(B3(Lb,BH),unsafe_allow_html=B)
		with Li:A.markdown("<p style='font-size:14px; font-weight:bold; margin-top:10px;'>🔥 Most Active Stocks (Volume & Traded Value)</p>",unsafe_allow_html=B);A.markdown(B3(Lc,JM),unsafe_allow_html=B)
		with Lj:A.markdown("<p style='font-size:14px; font-weight:bold; margin-top:10px;'>💰 Most Active by Traded Value</p>",unsafe_allow_html=B);A.markdown(B3(Ld,D_),unsafe_allow_html=B)
		with Lk:A.markdown("<p style='font-size:14px; font-weight:bold; margin-top:10px;'>💎 Highest Market Turnover</p>",unsafe_allow_html=B);A.markdown(B3(Le,CS),unsafe_allow_html=B)
		with Ll:A.markdown("<p style='font-size:14px; font-weight:bold; margin-top:10px;'>💰 Most Active (Highest Traded Value)</p>",unsafe_allow_html=B);A.markdown(B3(Lf,D_),unsafe_allow_html=B)
A.write(c)
def DM(row,actual_cols):
	...
	if F is not D:
		...
		elif F<=Am:B+=4;A.append(f"🟡 Manageable Debt (D/E={F:.2f})")
...
def Gx():
	...
	B=A.secrets[Dv]
	if Ay(B,G):B=EV.loads(B)
	C=[JC,JD];D=Gn.from_service_account_info(B,scopes=C);return EU.authorize(D)
Lm=JE
def Gy(client):
	try:
		A=client.open_by_key(Lm)
...
def Ln():
	...
	try:
		...
		for B in I:
			H=G(B.get(j,C)).strip()
			if H:F[H]={BI:G(B.get(D4,C)),A4:G(B.get(AM,C)),CV:G(B.get(D5,C)),Bj:G(B.get(E4,C)),D6:G(B.get(E5,C))}
...
def Lo(sym,cmp=C,note=C,bf_score=C,bf_grade=C):A.session_state.watchlist[sym]={A4:cmp,BI:note,CV:bf_score,Bj:bf_grade,D6:l.now().strftime('%Y-%m-%d %H:%M')}
def Gz(sym):A.session_state.watchlist.pop(sym,D)
if'watchlist_loaded'not in A.session_state:Ln();A.session_state.watchlist_loaded=B
def Lp(row_data,cols):
	N='sl_standard'
	def E(keys):
		for B in keys:
			for A in cols:
	...
	B=E([A4]);I=E(['52w high',D7,'52wk high']);J=E([JN,CU,'52wk low']);K=E([JP,'50dma']);L=E([Fw,'200dma']);A={A4:B,G1:I,G2:J,'dma50':K,'dma200':L}
	if B and I and J:F=(I-J)/52;A[E6]=b(F,2);A['sl_tight']=b(B-Am*F,2);A[N]=b(B-1.5*F,2);A['sl_wide']=b(B-2.*F,2);O=2.;H=B-A[N];A['target_1r']=b(B+H*Am,2);A['target_2r']=b(B+H*O,2);A['target_3r']=b(B+H*3.,2);A[D8]=b(K,2)if K else D;A['trail_sl_200dma']=b(L,2)if L else D;A['risk_pct']=b(H/B*100,2)if B else D
	return A
def Ee(history):
...
if A.sidebar.button('🧹 Clear All Filters',use_container_width=B):
	for Ef in AL(A.session_state.keys()):
		if Ef.startswith('filter_')or Ef in(JQ,JR,JS,JT):del A.session_state[Ef]
	A.session_state.grid_reset_token+=1;A.rerun()
A.sidebar.markdown(c)
A.sidebar.header('🔍 Global Search')
G_=A.sidebar.text_input('Search by Symbol, Name, etc...',key=JQ)
A.sidebar.markdown(c)
A.sidebar.header('📑 Select a Tab')
Lq=[BD,BE,CM,CN,CO,CP,CQ]
z=A.sidebar.selectbox('Choose sheet',Lq,key='filter_sheet')
...
if not Eg.empty:
	Eh=0;R=[A for A in Eg.columns if not A.startswith(AX)and not A.startswith(AY)];Lr=L7(z,R);Ei=L8.get(z)
	if Ei and Ei in R:Eh=R.index(Ei)
	else:
		for(BO,Ls)in Ds(R):
			if Ls.lower()in[D0,AZ,Dx,Dy,D1,Dz]:Eh=BO;break
	A.sidebar.markdown(c);A.sidebar.header('⚙️ Settings');Bz=A.sidebar.selectbox('Symbol Column (locked):',R,index=Eh,key='filter_symbol_col',disabled=B,help='Locked for consistency across sheets. To change it, edit LOCKED_SYMBOL_COLUMN near the top of the .py file.');H0=LO(Eg,Bz);K=H0.copy()
	if G_:Lt=K[R].astype(G).apply(lambda x:x.str.contains(G_,case=J,na=J)).any(axis=1);K=K[Lt]
	...
	if Ej!=AO:
		Ek=f"_bg_{Ej}"
		if Ek in K.columns:
			Lu=K[Ek].unique();El={C_:'⚪ White (Default)',x:'🟢 Green',AP:'🔴 Red',JU:'🟡 Yellow','#4285f4':'🔵 Blue',JV:'🟠 Orange','#b6d7a8':'🟩 Light Green','#f4cccc':'🟥 Light Red','#d9d2e9':'🟪 Light Purple'};Em=[]
			for Lv in Lu:
				En=G(Lv).lower()
			...
			if H1:
				Eo=[]
				for Ep in H1:
					for(Lw,Bx)in El.items():
						if Bx==Ep:Eo.append(Lw)
					if Ep.startswith(JW):Eo.append(Ep.replace(JW,C))
				K=K[K[Ek].str.lower().isin(Eo)]
	A.sidebar.markdown(c);A.sidebar.header('🎯 Categorical Filters');Lx=[A for A in R if AW(B in A.lower()for B in['cumulative average',G3,E7,JX,JY,G4,G5,G6,JZ,G7])]
	for DN in Lx:
		Ly=sorted([A for A in H0[DN].unique()if G(A).strip()!=C]);H2=A.sidebar.multiselect(f"Filter by {DN}:",options=Ly,key=f"filter_cat_{DN}")
		if H2:K=K[K[DN].isin(H2)]
	A.sidebar.markdown(c);A.sidebar.header('📈 DMA Trend Filter');Ce=A.sidebar.selectbox('Select DMA Condition:',[Ja,Jb,Jc,Jd,Je],key='filter_dma_trend')
	if Ce!=Ja:
		H3=U((A for A in R if JP in A.lower()),D);H4=U((A for A in R if'100 dma'in A.lower()),D);H5=U((A for A in R if Fw in A.lower()),D)
		if H3 and H5:
			DO=H.to_numeric(K[H3].astype(G).str.replace(BG,C,regex=B),errors=AN);DP=H.to_numeric(K[H5].astype(G).str.replace(BG,C,regex=B),errors=AN)
			if Ce==Jd:K=K[DO>DP]
			elif Ce==Je:K=K[DO<DP]
			elif H4:
				DQ=H.to_numeric(K[H4].astype(G).str.replace(BG,C,regex=B),errors=AN)
				if Ce==Jb:K=K[(DO<DQ)&(DQ<DP)]
				elif Ce==Jc:K=K[(DO>DQ)&(DQ>DP)]
	...
	Lz=[w,AM,J9,Jf,Jg,Jh,'Net Profit','EPS',Ji,Jj,'Enterprise Value','RSI','Delivery'];H6={Eq,Er,Es}
	for Cf in Lz:
		Et=U((A for A in R if Cf.lower()in A.lower()and A not in H6),D)
		if Et:K=DL(K,Et,A.sidebar);H6.add(Et)
	A.sidebar.markdown(c);A.sidebar.header('📅 Date Filters');H7=U((A for A in R if Jk in A.lower()),D);H8=U((A for A in R if Jl in A.lower()),D)
	...
	if A.session_state.watchlist:
		...
		for(DR,Eu)in AL(A.session_state.watchlist.items()):
			L_,M0=A.sidebar.columns([3,1]);L_.markdown(f"**{DR}** {"`"+Eu[A4]+"`"if Eu[A4]else C}<br><small style='color:gray'>{Eu.get(BI,C)[:35]}</small>",unsafe_allow_html=B)
			if M0.button('❌',key=f"wl_rm_{DR}",help=f"Remove {DR}"):Gz(DR);Ed();A.rerun()
		A.sidebar.markdown(C);M1=H.DataFrame([{j:B,AM:A[A4],D4:A[BI],D5:A.get(CV,C),E4:A.get(Bj,C),E5:A[D6]}for(B,A)in A.session_state.watchlist.items()]);HA=io.BytesIO()
		with H.ExcelWriter(HA,engine=D9)as M2:M1.to_excel(M2,index=J,sheet_name=G0)
	...
	if A.session_state.ai_history:
		A.sidebar.markdown(c);A.sidebar.header('🤖 AI History Export');A.sidebar.caption(f"{Q(A.session_state.ai_history)} analyses saved this session");M3=Ee(A.session_state.ai_history);A.sidebar.download_button('📥 Download All AI Results (Excel)',data=M3,file_name=f"AI_Analysis_{l.now().strftime(Jm)}.xlsx",mime=Bl,use_container_width=B)
	...
	B_=U((A for A in R if BH in A.lower()),D);M4=U((A for A in R if Jn in A.lower()or'prev'in A.lower()),D);A7=U((A for A in R if A4 in A.lower()),D);AJ=U((A for A in R if D2 in A.lower()),D);B4=U((A for A in R if E8 in A.lower()and'high'in A.lower()and BJ not in A.lower()and AC not in A.lower()),D);B5=U((A for A in R if E8 in A.lower()and'low'in A.lower()and BJ not in A.lower()and AC not in A.lower()),D);BQ=U((A for A in R if E3 in A.lower()),D);BR=U((A for A in R if'rsi'in A.lower()),D);Cg=U((A for A in R if G4 in A.lower()),D);BS=U((A for A in R if G5 in A.lower()),D);DS=U((A for A in R if G6 in A.lower()and A!=Cg and'dma'not in A.lower()),D);DT=U((A for A in R if'macd'in A.lower()),D);BT=U((A for A in R if G7 in A.lower()),D);BU=U((A for A in R if'diff'in A.lower()and'200'in A.lower()),D);HB=LB(z,R)
	if HB:
		for p in HB:
			if p not in BP:BP.append(p)
	else:
		for Cf in(B_,M4,A7,AJ,B4,B5):
			if Cf and Cf not in BP:BP.append(Cf)
	M5=[A for A in K.columns if A not in BP and not A.startswith(AX)and not A.startswith(AY)and A!=n];M6=[A for A in K.columns if A.startswith(AX)or A.startswith(AY)or A==n];M7=BP+M5+M6;K=K[M7];A.markdown(c)
	with A.expander(f"🚀 Executive Dashboard — {z}",expanded=B):
		...
		W=K;M8=Q(W);Ag=Af(W[AJ])if AJ and AJ in W.columns else H.Series(dtype=M);HC=Af(W[B_])if B_ and B_ in W.columns else H.Series(dtype=M);AT=Af(W[A7])if A7 and A7 in W.columns else H.Series(dtype=M);BV=Af(W[B4])if B4 and B4 in W.columns else H.Series(dtype=M);Au=Af(W[B5])if B5 and B5 in W.columns else H.Series(dtype=M);HD=Af(W[BR])if BR and BR in W.columns else H.Series(dtype=M);HE=Af(W[BQ])if BQ and BQ in W.columns else H.Series(dtype=M);Ev=U((A for A in R if G8 in A.lower()),D);HF=Af(W[Ev])if Ev and Ev in W.columns else H.Series(dtype=M);r=Af(W[BU])if BU and BU in W.columns else H.Series(dtype=M);Ew=U((A for A in R if CS in A.lower()),D);HG=Af(W[Ew])if Ew and Ew in W.columns else H.Series(dtype=M);HH=T((Ag>0).sum())if not Ag.empty else 0;Ex=T((Ag<0).sum())if not Ag.empty else 0;M9=T((Ag==0).sum())if not Ag.empty else 0;Op=M(Ag.mean())if Ag.notna().any()else i;Oq=HH/Ex if Ex>0 else D;Or=M(Ag.median())if Ag.notna().any()else D;Os=M(HC.sum())if HC.notna().any()else i;Ot=M(HF.sum())if HF.notna().any()else i;Ou=M(HG.sum())if HG.notna().any()else i;Ov=M(HD.mean())if HD.notna().any()else D;Ow=M(HE.mean())if HE.notna().any()else D;MA=T((r>0).sum())if r.notna().any()else 0;MB=T((r<0).sum())if r.notna().any()else 0;HI=0
		if BS and BS in W.columns:HI=T(W[BS].astype(G).str.contains('breakout|buy|bullish',case=J,na=J).sum())
		HJ=0
		if BT and BT in W.columns:HJ=T(W[BT].astype(G).str.contains('buy',case=J,na=J).sum())
		HK,HL=0,0;HM=0
		if AT.notna().any()and BV.notna().any():MC=AT/BV.replace(0,A6.nan)*100;HK=T((MC>=95).sum())
		...
		BW=A.columns(7);AU(BW[0],'📦 TOTAL STOCKS',f"{M8:,}");AU(BW[1],'🟢 ADVANCES',f"{HH:,}",bg=CW,fg=G9);AU(BW[2],'🔴 DECLINES',f"{Ex:,}",bg=Bm,fg=DA);AU(BW[3],'⚪ UNCHANGED',f"{M9:,}");AU(BW[4],'🕳️ NEAR 52W LOW (≤15%)',f"{HM:,}"if AT.notna().any()and Au.notna().any()else DB,bg=Bm,fg=DA);AU(BW[5],'🚀 BREAKOUTS',f"{HI:,}",bg=GA,fg='#e65100');AU(BW[6],'✅ BUY SIGNALS',f"{HJ:,}",bg=Jo,fg='#0d47a1');A.markdown("<div style='margin-top:8px;'></div>",unsafe_allow_html=B);DU=A.columns(4);AU(DU[0],'🏔️ NEAR 52W HIGH (≥95%)',f"{HK:,}",bg=CW,fg=G9);AU(DU[1],'🕳️ NEAR 52W LOW (≤5%)',f"{HL:,}",bg=Bm,fg=DA);AU(DU[2],'📉 BELOW 200 DMA',f"{MB:,}"if r.notna().any()else DB,bg=Bm,fg=DA);AU(DU[3],'🎯 ABOVE 200 DMA',f"{MA:,}"if r.notna().any()else DB,bg=CW,fg=G9);A.markdown(CX,unsafe_allow_html=B);DV={Jp:J,'modeBarButtons':[['toImage']]}
		def MD(frac):
		...
		def Ox(title_text,points,y_min,y_max,y_label,height=340):
			...
			for(R,(S,I,T))in Ds(E):U=(I-B)/O;K=AA(i,min(Am,U));V=MD(K);W=R/AA(N-1,1)*100;X=(1-K)*100;H+=f'<a href="{T}" target="_blank" title="{S}: {I:.2f}{y_label}" style="position:absolute; left:{W:.3f}%; top:{X:.3f}%; width:11px; height:11px; margin:-6px 0 0 -6px; border-radius:50%; background:{V}; display:block; border:1px solid rgba(255,255,255,0.75); box-shadow:0 0 1px rgba(0,0,0,0.35); cursor:pointer;"></a>'
			L=C
		...
		if n in W.columns:C0=W[n].astype(G).str.strip()
		else:C0=BX.astype(G).str.replace('<[^>]+>',C,regex=B).str.strip()
	...
	if BY is not D and Q(BY)>0 or Q(K)>0:
		...
		if F:
			...
			with A9[1]:Hf=f"https://charting.nseindia.com/?symbol={F}-EQ";A.markdown(f"**NSE Interactive Chart Frame** &nbsp;|&nbsp; [🌐 Open in Browser]({Hf})",unsafe_allow_html=J);A.caption(Bn);P.html(f'<iframe src="{Hf}" width="100%" height="{A8}" style="border:none; border-radius:5px;"></iframe>',height=A8+20)
			with A9[2]:Hg=f"https://www.equitypandit.com/historical-data/{F.lower()}";A.markdown(f"**EquityPandit Historical Matrix Data** &nbsp;|&nbsp; [🌐 Open in Browser]({Hg})");A.caption(Bn);P.html(f'<iframe src="{Hg}" width="100%" height="{A8}" style="border:none; border-radius:5px; background-color:white;"></iframe>',height=A8+20)
			with A9[3]:Hh=f"https://www.equitypandit.com/share-price/{F.lower()}#chart";A.markdown(f"**Bullish / Bearish Zone Indicator** &nbsp;|&nbsp; [🌐 Open in Browser]({Hh})");A.caption(Bn);P.html(f'<iframe src="{Hh}" width="100%" height="{A8}" style="border:none; border-radius:5px; background-color:white;"></iframe>',height=A8+20)
			with A9[4]:Hi=f"https://www.screener.in/company/{F}/consolidated/";A.markdown(f"**Screener Corporate Filings** &nbsp;|&nbsp; [🌐 Open in Browser]({Hi})");A.caption(Bn);P.html(f'<iframe src="{Hi}" width="100%" height="{A8}" style="border:none; border-radius:5px; background-color:white;"></iframe>',height=A8+20)
			with A9[5]:Hj=f"https://zerodha.com/markets/stocks/NSE/{F}/";A.markdown(f"**Zerodha Markets Financial Performance Metrics** &nbsp;|&nbsp; [🌐 Open in Browser]({Hj})");A.caption(Bn);P.html(f'<iframe src="{Hj}" width="100%" height="{A8}" style="border:none; border-radius:5px; background-color:white;"></iframe>',height=A8+20)
			with A9[6]:Hk=f"https://marketsmithindia.com/mstool/eval/{F.lower()}/evaluation.jsp";A.markdown(f"**MarketSmith India Institutional Trading Evaluation Engine** &nbsp;|&nbsp; [🌐 Open in Browser]({Hk})");A.caption(Bn);P.html(f'<iframe src="{Hk}" width="100%" height="{A8}" style="border:none; border-radius:5px; background-color:white;"></iframe>',height=A8+20)
			with A9[7]:Hl=f"https://www.tradingview.com/symbols/{F}/";A.markdown(f"**TradingView Comprehensive Asset Market Registry Summary Profile** &nbsp;|&nbsp; [🌐 Open in Browser]({Hl})");A.caption(Bn);P.html(f'<iframe src="{Hl}" width="100%" height="{A8}" style="border:none; border-radius:5px; background-color:white;"></iframe>',height=A8+20)
			with A9[8]:
				A.markdown(f"### 🤖 Ask AI About **{F}**")
				if not EY:A.warning(Jw)
				else:
					...
					if A.button('✨ Generate AI Analysis',use_container_width=B,key='btn_ai_analysis'):
						with A.spinner(f"Analyzing {F} with {Da}..."):
							try:F0={A:B for(A,B)in S.items()if not G(A).startswith(Bo)};Ck=f"""
...
''';Hn=EZ(Ck,Ma);A.session_state[GH]={DE:F,DF:Hn};A.markdown('### 📋 Your Custom Strategy Code:');A.write('Copy the code below and paste it into the TradingView Pine Editor.');A.markdown(Hn)
							except g as BZ:A.error(f"AI error: {BZ}")
					if A.session_state.get(GH,{}).get(DE)==F:Mc=A.session_state[GH][DF];Md=Ee([[F,'Pine Script',Hm,Mc,l.now().strftime(CR)]]);A.download_button('📥 Save Pine Script as Excel',data=Md,file_name=f"PineScript_{F}_{l.now().strftime(Bk)}.xlsx",mime=Bl,key='dl_pine_excel')
					A.markdown(c);A.markdown('**📋 Custom Rules Reference** — copy any rule and paste it into the Additional Custom Rules box above:');A.text(L3)
			with A9[10]:
				A.markdown(f"### 🔬 Bottom Fishing Analysis: **{F}**");A.caption('Scores this stock on 8 key criteria for buying from the bottom. Based entirely on your live sheet data.');Ho={A:B for(A,B)in S.items()if not G(A).startswith(Bo)};C1,F2,F3=DM(Ho,R);F4=Aa if C1>=75 else JU if C1>=55 else JV if C1>=35 else AP;A.markdown(f'''
...
""";F6=EZ(Ck,F5);A.session_state['last_bf_ai_result']={DE:F,DF:F6};A.session_state.ai_history.append([F,F5,'Bottom Fishing Deep Analysis',F6,l.now().strftime(CR)]);A.success('✅ AI Analysis Complete');A.markdown(F6)
							except g as BZ:A.error(f"AI error: {BZ}")
...
"""+'\n'.join(F3[:5])+f"\n\n🕒 {l.now().strftime(Jx)}\n📌 NSE Stock Dashboard";Mg=urllib.parse.quote(Hp);Mh=urllib.parse.quote(Hp);Mi,Mj=A.columns(2)
			...
			with A9[11]:
				A.markdown(f"### 🎯 GTT Order Calculator: **{F}**");A.caption('Auto-suggest Stop-Loss, Targets & ATR-based GTT levels from your live sheet data.');Mk={A:B for(A,B)in S.items()if not G(A).startswith(Bo)};AF=Lp(Mk,R)
				if not AF.get(A4):A.warning('⚠️ CMP column not found in sheet data. Cannot compute GTT levels.')
					...
					A.markdown(c);A.markdown('#### ⚙️ Customize ATR Multiplier');Mp,Mq=A.columns(2);Hq=Mp.number_input('Manual ATR Override (₹) — leave 0 to use auto',min_value=i,value=i,step=.5,key='gtt_manual_atr');Dc=Mq.selectbox('Risk-Reward Ratio:',['1:1','1:1.5','1:2','1:2.5','1:3'],index=2,key='gtt_rr_ratio');Mr=M(Dc.split(':')[1]);C2=Hq if Hq>0 else AF.get(E6,0)
					if C2 and C2>0:
						Hr=b(V-Am*C2,2);C3=b(V-1.5*C2,2);Hs=b(V-2.*C2,2);Dd=V-C3;De=b(V+Dd*Am,2);Df=b(V+Dd*Mr,2);Dg=b(V+Dd*3.,2);Ms=b(Dd/V*100,2);A.markdown('#### 🛡️ Stop-Loss Levels');F7=H.DataFrame([{EC:'Tight SL (1× ATR)',BK:Hr,ED:b((V-Hr)/V*100,2),EE:'Intraday / Scalp'},{EC:'Standard SL (1.5× ATR)',BK:C3,ED:b((V-C3)/V*100,2),EE:'Swing / BTST'},{EC:'Wide SL (2× ATR)',BK:Hs,ED:b((V-Hs)/V*100,2),EE:'Positional'}])
...
📦 Qty: {F8} shares | ₹{Hv:,.0f}
📊 ATR: ₹{C2:.2f} | R:R {Dc}
🕒 {l.now().strftime(Jx)}""";A.code(F9,language=C);Mx=urllib.parse.quote(F9);My=urllib.parse.quote(F9);Mz,M_=A.columns(2)
			...
			with A9[12]:
				A.markdown(f"### 📊 Watchlist Manager");Hw={A:B for(A,B)in S.items()if not G(A).startswith(Bo)};N0,N1,_=DM(Hw,R);N2=G(Hw.get(A7,C))if A7 else C;FA=F in A.session_state.watchlist;A.markdown(f"**Current Stock: {F}** {"✅ Already in Watchlist"if FA else C}");N3=A.text_input('📝 Note (optional):',value=A.session_state.watchlist.get(F,{}).get(BI,C),placeholder='e.g. Near 52W low, watching for breakout',key=f"wl_note_{F}");N4,N5=A.columns(2)
				with N4:
					if A.button(f"{"🔄 Update"if FA else"➕ Add"} {F} to Watchlist",use_container_width=B,key='wl_add_btn'):
						Lo(F,cmp=N2,note=N3,bf_score=G(N0),bf_grade=N1);N6=Ed()
						if N6:A.success(f"✅ {F} saved to Watchlist (Google Sheet updated!)")
						else:A.info(f"✅ {F} added to session Watchlist (Sheet write failed — check secrets).")
						A.rerun()
				with N5:
					if FA:
						if A.button(f"❌ Remove {F} from Watchlist",use_container_width=B,key='wl_rm_btn'):Gz(F);Ed();A.rerun()
				A.markdown(c);A.markdown('#### 🗂️ Your Full Watchlist')
				if A.session_state.watchlist:
					N7=[{j:B,'CMP (₹)':A[A4],D5:A.get(CV,C),GL:A.get(Bj,C),D4:A.get(BI,C),'Added':A.get(D6,C)}for(B,A)in A.session_state.watchlist.items()];Hx=H.DataFrame(N7);A.dataframe(Hx,use_container_width=B,hide_index=B);Hy=io.BytesIO()
					with H.ExcelWriter(Hy,engine=D9)as N8:Hx.to_excel(N8,index=J,sheet_name=G0)
					A.download_button('📥 Download Watchlist as Excel',data=Hy.getvalue(),file_name=f"Watchlist_{l.now().strftime(Bk)}.xlsx",mime=Bl,use_container_width=B,key='dl_wl_excel_tab');N9='\n'.join([f"• {B} — Score:{A.get(CV,C)} {A.get(Bj,C).split()[0]if A.get(Bj)else C} — {A.get(BI,C)[:30]}"for(B,A)in AL(A.session_state.watchlist.items())[:15]]);Hz=f"📊 *My NSE Watchlist*\n\n{N9}\n\n🕒 {l.now().strftime(EF)}";NA=urllib.parse.quote(Hz);NB=urllib.parse.quote(Hz);A.markdown(C);NC,ND=A.columns(2)
			...
			with A9[13]:
				A.markdown(f"### 📰 Latest News & Alerts: **{F}**");import urllib.request,urllib.parse,xml.etree.ElementTree as C4,datetime as l,email.utils
				def NE(pubdate_str):
					try:
				...
				with A.spinner(f"Fetching today's latest news for {F}..."):
					H_=NF(F,limit=10)
					if H_:
						for N in H_:h=Ab in N[L]or Ac in N[L]or Ad in N[L]or AH in N[L];s=Aa if h else B0;t=AR if h else AQ;A.markdown(f"- <a href='{N[f]}' target='_blank' style='text-decoration: none; color: inherit;'>{N[AD]}</a> <span style='color: {s}; font-weight: {t}; font-size: 0.85em;'>— 🕒 {N[L]}</span>",unsafe_allow_html=B);A.markdown("<hr style='margin: 0.5em 0; opacity: 0.2;'>",unsafe_allow_html=B)
					else:A.info(f"No recent news found for {F}.")
			with A9[0]:
				with A.expander(f"🕯️ Price Chart & Technical Indicators — {F}",expanded=B):
					NG=A.select_slider('History range:',options=['3mo','6mo','1y','2y','5y'],value='1y',key=f"chart_period_{F}")
					with A.spinner(f"Loading price history for {F}..."):m=LN(F,period=NG)
					if m.empty or BF not in m.columns:A.warning(f"⚠️ No historical price data available for **{F}** via Yahoo Finance (tried `{F}.NS`). The symbol may be delisted, renamed, or not tracked by Yahoo.")
					else:
						Ah=m[BF].squeeze().dropna();AK=M(Ah.iloc[-1]);Ba=M(Ah.iloc[-2])if Q(Ah)>1 else AK;Dh=(AK-Ba)/Ba*100 if Ba else i;I0=Ah.diff();NH=I0.clip(lower=0).rolling(14).mean();NI=(-I0.clip(upper=0)).rolling(14).mean();FB=100-100/(1+NH/NI.replace(0,M(A3)));Cm=FB.dropna().iloc[-1]if not FB.dropna().empty else D;NJ,NK=A.tabs(['Price + EMAs','RSI'])
						with NJ:
							NL=A.radio('Chart type',[Jy,'Line'],horizontal=B,key=f"chart_type_{F}");I1=Ah.diff();NM=I1.clip(lower=0).rolling(9).mean();NN=(-I1.clip(upper=0)).rolling(9).mean();Av=100-100/(1+NM/NN.replace(0,M(A3)));FC=Av.ewm(span=3,adjust=J).mean();I2=A6.arange(1,22,dtype=M);FD=Av.rolling(21).apply(lambda x:M(A6.dot(x,I2)/I2.sum()),raw=B);u=AL(m.index);I3=Av.values;Cn,I4=[],[];FE,I5=[],[]
							for BO in Dt(22,Q(Av)):
								FF,I6=I3[BO],I3[BO-1]
								if A6.isnan(FF)or A6.isnan(I6):continue
								if FF>=50 and I6<50:
									Di=Av.index[BO]
									if Di in Ah.index:Cn.append(Di);I4.append(M(Ah.loc[Di])*.993);FE.append(Di);I5.append(M(FF))
							...
							e=K_(rows=3,cols=1,shared_xaxes=B,row_heights=[.55,.25,.2],vertical_spacing=.03,specs=[[{GQ:'xy'}],[{GQ:'xy'}],[{GQ:'xy'}]])
							if NL==Jy:
							...
							for(NP,NQ,NR)in[(20,Jz,'EMA20'),(50,'#FF6D00','EMA50'),(200,'#2979FF','EMA200')]:NS=Ah.ewm(span=NP,adjust=J).mean();e.add_trace(O.Scatter(x=u,y=NS,name=NR,line=E(color=NQ,width=1.8)),row=1,col=1)
							I7=M(m[EM].max());I8=M(m[EN].min());e.add_hline(y=I7,line_dash=E9,line_color=J_,line_width=1.4,opacity=.85,row=1,col=1,annotation_text=f"52W High ₹{I7:,.2f}",annotation_position=K0,annotation_font=E(color=J_,size=13));e.add_hline(y=I8,line_dash=E9,line_color=K1,line_width=1.4,opacity=.85,row=1,col=1,annotation_text=f"52W Low ₹{I8:,.2f}",annotation_position='bottom right',annotation_font=E(color=K1,size=13))
							if Cn:e.add_trace(O.Scatter(x=Cn,y=I4,mode=EA,name='H-M Entry (RSI>50)',marker=E(color='lime',size=12,symbol=K2,line=E(color='white',width=1.5))),row=1,col=1)
							try:I9=m[w].squeeze();NT=m['Open'].squeeze();NU=m[BF].squeeze();NV=[GR if B>=A else GS for(A,B)in zip(NT.tolist(),NU.tolist())];e.add_trace(O.Bar(x=u,y=I9.tolist(),name=w,marker=E(color=NV,line=E(width=0)),opacity=.85,showlegend=J),row=3,col=1);NW=I9.rolling(20).mean();e.add_trace(O.Scatter(x=u,y=NW.tolist(),name='Vol Avg(20)',line=E(color='#616161',width=1.2,dash=DH)),row=3,col=1)
							except g:pass
							Dj=Av.reindex(Av.index);IA=H.Series(5e1,index=Av.index);NX=Dj.where(Dj>=50,5e1);e.add_trace(O.Scatter(x=u,y=IA.tolist(),line=E(width=0),mode=EO,showlegend=J,hoverinfo=EP),row=2,col=1);e.add_trace(O.Scatter(x=u,y=NX.tolist(),fill=K3,fillcolor='rgba(38,166,154,0.35)',line=E(width=0),mode=EO,showlegend=J,hoverinfo=EP),row=2,col=1);NY=Dj.where(Dj<=50,5e1);e.add_trace(O.Scatter(x=u,y=IA.tolist(),line=E(width=0),mode=EO,showlegend=J,hoverinfo=EP),row=2,col=1);e.add_trace(O.Scatter(x=u,y=NY.tolist(),fill=K3,fillcolor='rgba(239,83,80,0.35)',line=E(width=0),mode=EO,showlegend=J,hoverinfo=EP),row=2,col=1);e.add_trace(O.Scatter(x=u,y=Av.tolist(),name='RSI(9)',line=E(color='#1976D2',width=1.5)),row=2,col=1);e.add_trace(O.Scatter(x=u,y=FC.tolist(),name='EMA3',line=E(color='#4CAF50',width=1.5)),row=2,col=1);e.add_trace(O.Scatter(x=u,y=FD.tolist(),name='WMA21',line=E(color='#EF5350',width=1.5)),row=2,col=1)
							if FE:e.add_trace(O.Scatter(x=FE,y=I5,mode=EA,name='Entry (RSI panel)',showlegend=J,marker=E(color='lime',size=6,symbol=K2,line=E(color='white',width=1))),row=2,col=1)
							e.add_hline(y=70,line_dash=DH,line_color=GP,opacity=.5,row=2,col=1);e.add_hline(y=50,line_dash=E9,line_color='#888888',row=2,col=1,annotation_text='50',annotation_position='right');e.add_hline(y=30,line_dash=DH,line_color=Jz,opacity=.8,row=2,col=1,annotation_text='30',annotation_position='right');e.update_layout(template=q,height=950,title=E(text=f"{F} — Ultra HD Chart (Price, EMAs, H-M, Volume)",font=E(size=12,color='#0E1117',family=EQ)),margin=E(t=60,b=80,l=20,r=20),xaxis_rangeslider_visible=J,xaxis2_rangeslider_visible=J,xaxis3_rangeslider_visible=J,legend=E(orientation='h',y=-.15,x=.5,xanchor='center',yanchor='top',font=E(size=13,color=K4,family=EQ)),hovermode='x unified',font=E(size=13,color=K4,family=EQ),hoverlabel=E(font_size=14,font_family=EQ,bgcolor='rgba(255,255,255,0.95)'),plot_bgcolor=ER,paper_bgcolor=ER,bargap=.15);e.update_xaxes(showspikes=B,spikemode='across+toaxis',spikesnap='cursor',spikethickness=1.5,spikedash='solid',spikecolor='#808495',gridcolor=K5,linecolor=GT,tickfont=E(size=12,family=K6));e.update_yaxes(gridcolor=K5,zeroline=J,linecolor=GT,tickfont=E(size=12,family=K6));e.update_yaxes(range=[0,100],row=2,col=1);e.update_yaxes(title_text=BK,title_font=E(size=14,weight=AR),row=1,col=1);e.update_yaxes(title_text='RSI / H-M',title_font=E(size=14,weight=AR),row=2,col=1);e.update_yaxes(title_text=w,title_font=E(size=14,weight=AR),row=3,col=1);NZ={Jp:J,'responsive':B,'toImageButtonOptions':{'format':'png','filename':f"{F}_Ultra_HD_Analysis",'height':1080,'width':1920,'scale':6},'modeBarButtonsToAdd':['drawline','drawopenpath','drawrect','eraseshape']};A.plotly_chart(e,use_container_width=B,key=f"price_ema_chart_{F}",config=NZ)
							...
							if z!=BE:
								Dk=DK(BE)
								if not Dk.empty:
									Na=[A for A in Dk.columns if not A.startswith(AX)and not A.startswith(AY)];IB=U((A for A in Na if A.lower()in[D0,AZ,Dx,Dy,D1,Dz]),D)
									if IB:
										IC=Dk[Dk[IB].astype(G).str.strip()==F]
										if not IC.empty:Nb=IC.iloc[0].to_dict();X={A:B for(A,B)in Nb.items()if not G(A).startswith(AX)and not G(A).startswith(AY)and G(A)!=n}
							def Y(row,primary_dict,*K):
								def B(r_data):
									...
									H=[A for A in H if not G(A).startswith(AX)and not G(A).startswith(AY)and G(A)!=n]
									for L in K:
										I=L.lower().strip()
										for F in H:
							...
							def FJ(title,fields):D=C.join(ID(A,Y(S,X,*B))for(A,B)in fields);A.markdown(f"<div style='font-size:13px;font-weight:700;color:#1565C0;margin:14px 0 6px 0;'>{title}</div><div style='display:flex;flex-wrap:wrap;gap:8px;'>{D}</div>",unsafe_allow_html=B)
						with NK:Nc=AL(m.index);B7=O.Figure();B7.add_trace(O.Scatter(x=Nc,y=FB,name=K7,line=E(color='#AB47BC',width=2)));B7.add_hline(y=70,line_dash=DH,line_color=GP,opacity=.6);B7.add_hline(y=30,line_dash=DH,line_color=GO,opacity=.6);B7.add_hrect(y0=45,y1=65,fillcolor=GO,opacity=.06,line_width=0,annotation_text='Ideal entry 45-65',annotation_position=K0);B7.update_layout(template=q,height=280,yaxis=E(range=[0,100]),margin=E(t=30,b=20),plot_bgcolor=ER,paper_bgcolor=ER,font=E(color='#1A1A1A'));B7.update_xaxes(gridcolor=K8);B7.update_yaxes(gridcolor=K8);A.plotly_chart(B7,use_container_width=B,key=f"rsi14_chart_{F}")
				A.markdown("<hr style='margin:16px 0 4px 0;opacity:0.25;'>",unsafe_allow_html=B)
				with A.expander(f"📋 {F} — Google Sheet Data",expanded=B):
					def Nd(title,items):D=C.join(ID(A,B)for(A,B)in items);A.markdown(f"<div style='font-size:13px;font-weight:700;color:#1565C0;margin:14px 0 6px 0;'>{title}</div><div style='display:flex;flex-wrap:wrap;gap:8px;'>{D}</div>",unsafe_allow_html=B)
					Ne='▲'if Dh>=0 else'▼';Nf='#00A152'if Dh>=0 else'#D32F2F';Nd('📊 Price Snapshot',[(K9,f"₹{AK:,.2f} <span style='color:{Nf};font-size:12px;'>{Ne} {Dh:+.2f}%</span>"),(Ak,f"₹{M(m[EM].max()):,.2f}"),(Al,f"₹{M(m[EN].min()):,.2f}"),(K7,f"{Cm:.1f}"if Cm is not D else'–')])
					with A.expander('📋 Company Price Dashboard',expanded=J):FJ('🏢 Company Info',[('Company Name',[Ju,Jv]),(GU,[E7,G3]),(Aj,[KA,KB,E3]),('52W High Date',[Jk,'52 week high date']),('52W Low Date',[Jl,'52 week low date']),(w,[BH]),(D3,[CS])]);FJ('📡 Signals & System Output',[(JA,[JX]),('Difference from 200 DMA',['difference from 200 dma','differance from 200 dma']),('CAR Rating',['cumulative average rule (car) rating','car rating']),('Start GTT Order',[JY,'gtt order']),(Bp,[G4]),(Bq,[G5]),(Br,[G6]),(Bs,[JZ]),(Bt,[G7])]);FJ('💰 Fundamentals',[(Jh,['face value']),('Total Equity Capital',[GV]),(Jj,[G8]),('EPS',['eps']),(Ji,['ronw']),(Jf,[Fx,Fy]),(Jg,[KC,KD]),('Pledged %',[Fz,F_]),('D/E Ratio',[JO,'de ratio']),('Net Sales (Cr)',[E2]),('Net Profit (Cr.)',[E1]),('Reserves (Cr)',[GW]),('Total Debt (Cr)',[GX]),('Inventory (Cr)',[GY]),('Cash & Equiv (Cr)',[GZ,Ga,Gb]),('Operating Cash Flow (Cr)',['operating cash flow']),('Trade Receivables (Cr)',[Gc]),('Trade Payables (Cr)',[Gd]),('Fixed Assets/Net PPE (Cr)',[Ge,Gf]),('Total Assets (Cr)',[Gg]),('Open (₹)',['open price','open (','open']),('High (₹)',['day high','high price','high (']),('Low (₹)',['day low','low price','low (']),('Prev Close (₹)',['prev close','previous close',Jn]),('Price Change (₹)',['price change','change (','change in price']),('% Change',['% change',D2,'change %']),('Shares Outstanding (Cr)',['shares outstanding']),('Book Value (₹/share)',['book value']),('Public %',['public %','public holding']),('FII %',['fii %','fii holding','fii']),('DII %',['dii %','dii holding','dii'])])
					def d(raw):
					...
					if Bb is not D and B8 is not D and 0<B8<Bb:IZ=Bb-B8;FP=B8/Bb*100;Ia=O.Figure(O.Sankey(arrangement=Bu,textfont=E(color=Ao,size=13,family=Bv),node=E(pad=30,thickness=18,line=E(color=Bw,width=.5),label=[f"Net Sales<br>₹{Bb:,.2f} Cr (100%)",f"Net Profit<br>₹{B8:,.2f} Cr ({FP:.1f}%)",f"Total Expenses<br>₹{IZ:,.2f} Cr ({100-FP:.1f}%)"],color=[AE,x,AP]),link=E(source=[0,0],target=[1,2],value=[B8,IZ],color=['rgba(15,157,88,0.35)','rgba(234,67,53,0.35)'])));Ia.update_layout(title=f"💰 Revenue & Expenses Flow — {F} (Net Margin {FP:.1f}%)",template=q,height=320,margin=E(t=45,b=10,l=10,r=10),font=E(size=12));A.plotly_chart(Ia,use_container_width=B,key=f"sankey_{F}");A.caption('Based on Net Sales / Net Profit from the Fundamentals data above. "Total Expenses" is the remainder (Net Sales − Net Profit) — your sheet doesn\'t carry a Cost-of-Revenue/Opex breakdown, so a multi-stage flow (Gross → Operating → Net) isn\'t available for this stock.')
					elif Bb is not D and B8 is not D:A.info(f"Revenue & Expenses flow needs a normal profitable split (0 < Net Profit < Net Sales). {F} currently shows Net Sales ₹{Bb:,.2f} Cr and Net Profit ₹{B8:,.2f} Cr, which doesn't fit a simple flow diagram (e.g. a net loss).")
					else:A.info("Net Sales / Net Profit not available for this stock, so the Revenue & Expenses flow can't be built.")
					Ni=d(Y(S,X,GV));Nj=d(Y(S,X,GW));Nk=d(Y(S,X,GX));Nl=d(Y(S,X,Gd));Nm=[(KF,Ni,AE),(KG,Nj,x),(KH,Nk,AP),(KI,Nl,KJ)];B9=[(B,A,C)for(B,A,C)in Nm if A is not D and A>0]
					if Q(B9)>=2:Ib=sum(B for(A,B,A)in B9);Ic=O.Figure(O.Sankey(arrangement=Bu,textfont=E(color=Ao,size=13,family=Bv),node=E(pad=30,thickness=18,line=E(color=Bw,width=.5),label=[f"Total Financing<br>₹{Ib:,.2f} Cr (100%)"]+[f"{B}<br>₹{A:,.2f} Cr ({A/Ib*100:.1f}%)"for(B,A,C)in B9],color=[KK]+[B for(A,A,B)in B9]),link=E(source=[0]*Q(B9),target=AL(Dt(1,Q(B9)+1)),value=[B for(A,B,A)in B9],color=[A0(B)for(A,A,B)in B9])));Ic.update_layout(title=f"🏗️ Capital Structure — How {F} Is Financed",template=q,height=300,margin=E(t=45,b=10,l=10,r=10),font=E(size=12));A.plotly_chart(Ic,use_container_width=B,key=f"sankey_capstruct_{F}");A.caption('Equity Capital + Reserves + Total Debt + Trade Payables, from the Fundamentals data above.')
					...
					CA=d(Y(S,X,Gg));Nn=[(KL,d(Y(S,X,Ge,Gf)),KM),(KN,d(Y(S,X,GY)),A_),(KO,d(Y(S,X,Gc)),KP),(KQ,d(Y(S,X,GZ,Ga,Gb)),AE)];FQ=[(B,A,C)for(B,A,C)in Nn if A is not D and A>=0]
					if CA is not D and CA>0 and FQ:
						Id=sum(B for(A,B,A)in FQ);FR=CA-Id
						if FR>=0:CB=FQ+([(KR,FR,ES)]if FR>0 else[]);Ie=O.Figure(O.Sankey(arrangement=Bu,textfont=E(color=Ao,size=13,family=Bv),node=E(pad=30,thickness=18,line=E(color=Bw,width=.5),label=[f"Total Assets<br>₹{CA:,.2f} Cr (100%)"]+[f"{B}<br>₹{A:,.2f} Cr ({A/CA*100:.1f}%)"for(B,A,C)in CB],color=[Cc]+[B for(A,A,B)in CB]),link=E(source=[0]*Q(CB),target=AL(Dt(1,Q(CB)+1)),value=[B for(A,B,A)in CB],color=[A0(B)for(A,A,B)in CB])));Ie.update_layout(title=f"📦 Asset Deployment — Where {F}'s Assets Sit",template=q,height=340,margin=E(t=45,b=10,l=10,r=10),font=E(size=12));A.plotly_chart(Ie,use_container_width=B,key=f"sankey_assets_{F}");A.caption('Fixed Assets, Inventory, Trade Receivables and Cash & Equivalents from the Fundamentals data above. "Other Assets" is the gap versus reported Total Assets (e.g. intangibles, investments, or other items your sheet doesn\'t itemize).')
						else:A.info(f"{F}'s itemized asset categories (₹{Id:,.2f} Cr) add up to more than the reported Total Assets (₹{CA:,.2f} Cr) — likely a data mismatch between sheet rows, so the Asset Deployment flow isn't shown to avoid a misleading chart.")
					else:A.info("Total Assets / asset-category data not available for this stock, so the Asset Deployment flow can't be built.")
					FS,If,Dm=[],[],[];CC,CD,CE,CF=[],[],[],[]
					def Bc(label,color,col_x):FS.append(label);If.append(color);Dm.append(col_x);return Q(FS)-1
					No,Np,Ig,FT=.001,.24,.5,.999;Nq=[(KF,d(Y(S,X,GV)),AE),(KG,d(Y(S,X,GW)),x),(KH,d(Y(S,X,GX)),AP),(KI,d(Y(S,X,Gd)),KJ)];FU=[(B,A,C)for(B,A,C)in Nq if A is not D and A>0];Ih=Q(FU)>=2;BA=D
					if Ih:
						Dn=sum(B for(A,B,A)in FU);BA=Bc(f"Total Financing<br>₹{Dn:,.2f} Cr (100%)",KK,Np)
						for(Aw,AG,Cq)in FU:u=Bc(f"{Aw}<br>₹{AG:,.2f} Cr ({AG/Dn*100:.1f}%)",Cq,No);CC.append(u);CD.append(BA);CE.append(AG);CF.append(A0(Cq))
					Bd=d(Y(S,X,Gg));Nr=[(KL,d(Y(S,X,Ge,Gf)),KM),(KN,d(Y(S,X,GY)),A_),(KO,d(Y(S,X,Gc)),KP),(KQ,d(Y(S,X,GZ,Ga,Gb)),AE)];FV=[(B,A,C)for(B,A,C)in Nr if A is not D and A>=0];Do=Bd is not D and Bd>0 and bool(FV)
					if Do:Ns=sum(B for(A,B,A)in FV);FW=Bd-Ns;Do=FW>=0
					if Do:
						Nt=FV+([(KR,FW,ES)]if FW>0 else[]);Nu=f" ({Bd/Dn*100:.1f}%)"if BA is not D else KS;Ii=Bc(f"Total Assets<br>₹{Bd:,.2f} Cr{Nu}",Cc,Ig)
						if BA is not D:CC.append(BA);CD.append(Ii);CE.append(Bd);CF.append(A0(Cc))
						for(Aw,AG,Cq)in Nt:u=Bc(f"{Aw}<br>₹{AG:,.2f} Cr ({AG/Bd*100:.1f}%)",Cq,FT);CC.append(Ii);CD.append(u);CE.append(AG);CF.append(A0(Cq))
					Be=d(Y(S,X,E2));CG=d(Y(S,X,E1));Ij=Be is not D and CG is not D and 0<CG<Be
					if Ij:
						Ik=Be-CG;Nv=f" ({Be/Dn*100:.1f}%)"if BA is not D else KS;FX=Bc(f"Net Sales<br>₹{Be:,.2f} Cr{Nv}",AE,Ig)
						if BA is not D:CC.append(BA);CD.append(FX);CE.append(Be);CF.append(A0(AE))
						Il=CG/Be*100;Nw=Bc(f"Net Profit<br>₹{CG:,.2f} Cr ({Il:.1f}%)",x,FT);Nx=Bc(f"Total Expenses<br>₹{Ik:,.2f} Cr ({100-Il:.1f}%)",AP,FT);CC+=[FX,FX];CD+=[Nw,Nx];CE+=[CG,Ik];CF+=[A0(x),A0(AP)]
					Ny=sum([Ih,Do,Ij])
					if Ny>0:
						from collections import defaultdict as Im;In=Im(T)
						for Cr in Dm:In[Cr]+=1
						Io=Im(T);Ip=[]
						for Cr in Dm:Aw=In[Cr];BO=Io[Cr];Io[Cr]+=1;Ip.append(b((BO+.5)/Aw,4)if Aw>1 else .5)
						Iq=O.Figure(O.Sankey(arrangement=Bu,textfont=E(color=Ao,size=13,family=Bv),node=E(pad=22,thickness=18,line=E(color=Bw,width=.5),label=FS,color=If,x=Dm,y=Ip),link=E(source=CC,target=CD,value=CE,color=CF)));Iq.update_layout(title=f"💎 Combined Money Flow — {F} (Financing → Assets / Revenue, merged)",template=q,height=560,margin=E(t=45,b=10,l=10,r=10),font=E(size=12));A.plotly_chart(Iq,use_container_width=B,key=f"sankey_merged_{F}");A.caption("All money-related flows merged into one chart: financing sources (Equity + Reserves + Debt + Trade Payables) feed Total Financing, which splits into two parallel paths — Total Assets (incl. Trade Receivables) and Net Sales → Net Profit / Total Expenses. It's drawn as two branches off one hub, rather than one long chain, because Total Assets and Net Sales are different kinds of totals (balance sheet vs. P&L) that don't feed into each other. Trade Payables now also appears in the 🏗️ Capital Structure chart above.")
	...
	def Nz():
		...
		with z:f=A.radio(GC,[GD,CY,CZ],horizontal=B,help=KT,key='perf_matrix_sizing_mode')
		...
		with A2:j=A.text_input('🔍 Filter stocks inside this matrix...',placeholder=Jt,key=JS)
		V={}
		for L in g:
			if L==w:
				if B_:V[L]=B_
		...
		if V:
			...
			for(AR,N)in K.iterrows():
				o=G(N.get(n,C)).strip();A5=N.get(A7,C)if A7 else C;A6=f"https://charting.nseindia.com/?symbol={o}-EQ";A8=f'<a href="{A6}" target="_blank" style="text-decoration:none; color:#000000; font-weight:bold;">{o}</a>';F={W:A8,a:A5}
				for(L,A9)in V.items():
					p=G(N.get(A9,'0')).replace(AC,C).replace(AB,C).strip()
					try:F[L]=M(p)if p not in[C,A3,AO]else i
					except AV:F[L]=i
				if BQ:
					q=G(N.get(BQ,'0')).replace(AC,C).replace(AB,C).strip()
					try:F[Aj]=M(q)if q not in[C,A3,AO]else i
					except AV:F[Aj]=i
				if BR:
					r=G(N.get(BR,C)).replace(AC,C).replace(AB,C).strip()
					try:F[BL]=M(r)if r not in[C,A3,AO]else D
					except AV:F[BL]=D
				if BU:
					s=G(N.get(BU,C)).replace(AC,C).replace(AB,C).strip()
					try:F[B1]=M(s)if s not in[C,A3,AO]else D
					except AV:F[B1]=D
				if B4:
					t=G(N.get(B4,C)).replace(AB,C).strip()
					try:F[Ak]=M(t)if t not in[C,A3,AO]else D
					except AV:F[Ak]=D
				if B5:
					u=G(N.get(B5,C)).replace(AB,C).strip()
					try:F[Al]=M(u)if u not in[C,A3,AO]else D
					except AV:F[Al]=D
				if Cg:F[Bp]=G(N.get(Cg,C)).strip()
				if BS:F[Bq]=G(N.get(BS,C)).strip()
				if DS:F[Br]=G(N.get(DS,C)).strip()
				if DT:F[Bs]=G(N.get(DT,C)).strip()
				if BT:F[Bt]=G(N.get(BT,C)).strip()
				AD={A:B for(A,B)in N.items()if not G(A).startswith(Bo)};AE,AF,_=DM(AD,R);F[b]=AE;F[d]=AF;m.append(F)
			P=H.DataFrame(m)
			...
			for I in E.columns:
				if I in(e,):continue
				if f==CY and Q(E)>0:Y=By(E.iloc[0][I]);Z=Q(G(I));S=T(AA(Y,Z)*7+22)
				elif f==CZ and Q(E)>1:Y=By(E.iloc[1][I]);Z=Q(G(I));S=T(AA(Y,Z)*7+22)
	...
	def N_():
		...
		with o:a=A.radio(GC,[GD,CY,CZ],horizontal=B,help=KT,key='bf_scanner_sizing_mode')
		...
		with r:b=A.text_input(Js,placeholder='e.g. WIPRO',key=JT)
		L=[]
		for(AJ,d)in K.iterrows():
			E={A:B for(A,B)in d.items()if not G(A).startswith(Bo)};e,t,u=DM(E,R)
			if e>=X:
				f=G(d.get(n,C)).strip();v=E.get(A7,C)if A7 else C;g=U((A for A in R if E7 in A.lower()),D);w=E.get(g,C)if g else C;x=f"https://charting.nseindia.com/?symbol={f}-EQ";y=f'<a href="{x}" target="_blank" style="text-decoration:none; color:#000000; font-weight:bold;">{f}</a>';P=D
				if BQ:
					h=G(E.get(BQ,C)).replace(AC,C).replace(AB,C).strip()
					try:P=M(h)if h not in[C,A3,AO]else D
					except AV:P=D
				z=G(E.get(BR,C)).strip()if BR else k;A0=G(E.get(BU,C)).strip()if BU else k;A1=G(E.get(B4,C)).strip()if B4 else k;A2=G(E.get(B5,C)).strip()if B5 else k;A4=G(E.get(Cg,C)).strip()if Cg else k;A5=G(E.get(BS,C)).strip()if BS else k;A6=G(E.get(DS,C)).strip()if DS else k;A8=G(E.get(DT,C)).strip()if DT else k;A9=G(E.get(BT,C)).strip()if BT else k;L.append({j:y,W:e,GL:t,AM:v,BL:z,Aj:f"{P:.2f}%"if P is not D else k,B1:A0,Ak:A1,Al:A2,Bp:A4,Bq:A5,Br:A6,Bs:A8,Bt:A9,GU:G(w)[:30],m:' | '.join(u[:3])})
		if b:L=[A for A in L if b.upper()in re.sub(Dw,C,A[j]).upper()]
		L.sort(key=lambda x:x[W],reverse=s==l)
		if L:
			A.success(f"✅ Found **{Q(L)}** stocks matching your bottom-fishing criteria (score ≥ {X})");I=H.DataFrame(L);N=EX.from_dataframe(I);N.configure_default_column(filter=B,sortable=B,resizable=B,floatingFilter=J,flex=0);AD=B2(KU);AE=B2(KV);AF={j:120,W:90,GL:160,AM:100,Aj:110,GU:200,m:400,BL:100,B1:140,Ak:110,Al:110,Bp:120,Bq:130,Br:130,Bs:130,Bt:130}
			for F in I.columns:
				if a==CY and Q(I)>0:Y=By(I.iloc[0][F]);Z=Q(G(F));O=T(AA(Y,Z)*7+22)
				elif a==CZ and Q(I)>1:Y=By(I.iloc[1][F]);Z=Q(G(F));O=T(AA(Y,Z)*7+22)
				else:O=AF.get(F,120)
				S=DD if F==j else D;V=AA(70,min(O,90))
				if F==W:N.configure_column(F,width=O,minWidth=V,pinned=S,cellStyle=AD)
				elif F==j:N.configure_column(F,width=O,minWidth=V,pinned=S,cellRenderer=DX)
				elif F in(Bp,Bq,Br,Bs,Bt):N.configure_column(F,width=O,minWidth=V,pinned=S,cellStyle=AE)
				else:N.configure_column(F,width=O,minWidth=V,pinned=S)
	...
	N_()
	if AJ:
		A.markdown(c);A.markdown('### 🏆 Top 10 & Bottom 10 Performers (Daily badges)');CH=K.copy();CH[AJ]=H.to_numeric(CH[AJ].astype(G).str.replace(BG,C,regex=B),errors=AN);CH=CH.dropna(subset=[AJ]);O0=CH.nlargest(10,AJ);O1=CH.nsmallest(10,AJ);Ar,As=A.columns(2)
		with Ar:
			Ir="<h4 style='margin-top:0px; margin-bottom:8px;'>⬆️ Top 10 (Daily)</h4>"
			for(_,Bf)in O0.iterrows():
				Cs=G(Bf.get(n,C)).strip();AG=Bf[AJ];Ct=Bf.get(A7,C)if A7 else C
				try:Bg=M(G(Ct).replace(AB,C).strip());FY=M(AG);FZ=Bg/(1+FY/100);Fa=Bg-FZ;Cu=f"<span style='font-size: 0.85em; opacity: 0.75; margin-right: 6px;'>+{Fa:,.2f}</span>";Cv=f"₹{Bg:,.2f}"
				except:Cv=f"₹{Ct}";Cu=C
				Fb=f"https://charting.nseindia.com/?symbol={Cs}-EQ";Ir+=f"<a href='{Fb}' target='_blank' style='text-decoration:none;'><div style='background-color:#16e37f; padding:6px 12px; margin-bottom:4px; border-radius:5px; color:#000000; font-weight:bold; display:flex; justify-content:space-between;'><span>{Cs}: +{AG}%</span><span>{Cu}{Cv}</span></div></a>"
		...
		with As:
			...
			for(_,Bf)in O1.iterrows():
				...
				Fb=f"https://charting.nseindia.com/?symbol={Cs}-EQ";Is+=f"<a href='{Fb}' target='_blank' style='text-decoration:none;'><div style='background-color:#f39991; padding:6px 12px; margin-bottom:4px; border-radius:5px; color:#000000; font-weight:bold; display:flex; justify-content:space-between;'><span>{Cs}: {AG}%</span><span>{Cu}{Cv}</span></div></a>"
	...
	def O2(symbol,limit=10):
		try:
			...
			for A in Q.findall(DG):
				...
				if U<=15.:V=Dp(I);E.append({AD:f"🚨 **[ALERT]** {F}",f:S,L:V,AI:G,KW:F})
			E.sort(key=lambda x:x[AI],reverse=B);return E[:limit]
		except g:return[]
	@A.cache_data(ttl=600)
	def O3(symbol,limit=5):
		try:
			...
			P=C4.fromstring(O);E=[];Q=[D7,EG,CU,EH,EI,EJ,EK,EL,KX,KY]
	...
	O6={'RELIANCE':'500325','TCS':'532540','HDFCBANK':'500180','INFY':Kc,'ICICIBANK':'532174','HINDUNILVR':'500696','SBIN':'500112','BHARTIARTL':'532454','BAJFINANCE':'500034','KOTAKBANK':'500247','LT':'500510','HCLTECH':'532281','AXISBANK':'532215','ASIANPAINT':'500820','MARUTI':'532500',KZ:Kd,'TITAN':'500114','ULTRACEMCO':'532538','ONGC':'500312','NTPC':'532555','POWERGRID':'532898','WIPRO':'507685','NESTLEIND':'500790','JSWSTEEL':'500228','TATASTEEL':'500470','TATAMOTORS':'500570','TECHM':'532755','GRASIM':'500300','ADANIENT':'512599','ADANIPORTS':'532921','COALINDIA':'533278','DIVISLAB':Ke,'DRREDDY':'500124','EICHERMOT':'505200','BAJAJFINSV':'532978','BAJAJ-AUTO':'532977','CIPLA':'500087','BRITANNIA':'500825','HEROMOTOCO':'500182',Ka:Kf,'HINDALCO':'500440','UPL':'512070','TATACONSUM':'500800','SBILIFE':'540719','HDFCLIFE':'540777','INDUSINDBK':'532187','BPCL':'500547','IOC':'530965','M&M':'500520','PIDILITIND':'500331','SIEMENS':'500550','HAVELLS':'517354','VOLTAS':'500575','AMBUJACEM':'500425','ACC':'500410','SHREECEM':'500387','RAMCOCEM':Kg,Kb:Kh,'JKCEMENT':'532644','STAR':Ki,'TVSMOTOR':'532343','BOSCHLTD':'500530','MUTHOOTFIN':'533398','CHOLAFIN':'500443','BAJAJHLDNG':'500490','TORNTPHARM':Kj,'AUROPHARMA':'524208','LUPIN':'500257','BIOCON':'532523','ALKEM':'539523','IPCALAB':'530827','GLAXO':'500660','ABBOTINDIA':'500488','PFIZER':'500680','SANOFI':'500674','MCDOWELL-N':'532432','ITC':'500875','GODFRYPHLP':'500163','COLPAL':'500830','DABUR':'500096','MARICO':'531642','GODREJCP':'532424','HINDPETRO':'500104','CASTROLIND':'500870','INDIGO':'521737','INTERGLOBE':'539448','SPICEJET':'500285','IRCTC':'542830','CONCOR':'531344','ADANIGREEN':'541450','ADANITRANS':'539254','TATAPOWER':'500400','TORNTPOWER':'532779','CESC':'500084','NHPC':'533098','SJVN':'533206','PFC':'532810','RECLTD':'532955','IRFC':'543257','ZOMATO':'543320','NYKAA':'543384','PAYTM':'543396','POLICYBZR':'543390','DELHIVERY':'543529','CARTRADE':'543202','RVNL':'542649','IRCON':'541956','NBCC':'534309','HUDCO':'540530','MMTC':Kk,'MTNL':'500108','BEL':'500049','HAL':'541154','COCHINSHIP':'526235','MAZAGON':'543237','GRSE':'542351','MIDHANI':'541195','BEML':'500048','BHEL':'500103','SAIL':'500113','NMDC':'526371','MOIL':'533286','NATIONALUM':'532234','HINDZINC':'500188','VEDL':'500295','GMRINFRA':'532754','NHAI':'500253','IRB':'532947','ASHOKLEY':'500477','ESCORTS':'500495','FORCE':'517168','SML':'513275','MOTHERSON':'517334','MINDAIND':'532539','ENDURANCE':'540350','BALKRISIND':'502355','APOLLOTYRE':'500877','MRF':'500290','CEATLTD':'500878','JK TYRE':'530007','INOXWIND':'539083','SUZLON':'532667','RPOWER':'500390','JPPOWER':'532627','FEDERALBNK':'500469','IDFCFIRSTB':'539437','BANDHANBNK':'541153','RBLBANK':'540065','DCBBANK':'532772','KTKBANK':Kl,'SOUTHBANK':'532218','CANBK':'532483','BANKBARODA':'532134','UNIONBANK':'532477','INDIANB':'532814','UCOBANK':'532505','CENTRALBK':'532885','MAHABANK':'532525','J&KBANK':Kl,'PNB':'532461','IOB':'532388','BANKINDIA':'532149','DENABANK':'532121','SYNDIBANK':'532276','VIJAYABANK':'532245','ORIENTBANK':'500315','CORPBANK':'532179','ANDHRABANK':'532418','ALLAHABAD':Km,'ALBK':Km,'MFSL':'542299','HDFCAMC':'541530','NIPPONLIFE':'543171','UTIAMC':'543238','ABCAPITAL':'540691','ANGELONE':'543235','ICICIGI':'540716','GICRE':'540755','NIACL':'540769','STAR':Ki,'CROMPTON':'539876','ORIENTELEC':'531637','BLUESTAR':'500067','WHIRLPOOL':'500238','VGUARD':'532953','BAJAJEL':'500031','CERA':'532443','HINDWARE':'509820','HSIL':'509675','KAJARIACER':'500233','SOMANYCER':'532622','GRINDWELL':'506076','CARBORUNIV':'513375','ASTRAL':'532830','FINOLEX':'500940','SUPREMEIND':'509930','BERGER':'509480','KANSAINER':'500165','AKZOINDIA':'500710','INDIACEM':'530005','RAMCOIND':Kg,Kb:Kh,'HEIDELBERG':'500292','PRISM':'500338','BIRLACORPN':'500335','ORIENTCEM':'502420','SAGCEM':'502090','STARCEMENT':'540575','JKLAKSHMI':'500380','NUVOCO':'543334','ZYDUSLIFE':'532321','TORNTPHAR':Kj,'NATCOPHAR':'524816','GRANULES':'532482','LAURUS':Kn,'STRIDES':'532531','AJANTPHAR':'532331','CAPLIPOINT':'539266','DIVI':Ke,KZ:Kd,'GLAND':'543245','SEQUENT':'543225','METROPOLIS':'542650','DRLAL':'532259','THYROCARE':'539871','KRSNAA':'543328','VIJAYA':'532542','MAXHEALTH':'543220','KIMS':'543308','ASTER':'540975','FORTIS':'532843','NHOSPIT':'532526',Ka:Kf,'NARAYANA':'539551','YATHARTH':'544120','RAINBOW':'543524','SUVENPHAR':'530239','LAURUSLABS':Kn,'SOLARA':'541540','SHILPAMED':'530879','PERSISTENT':'533179','MINDTREE':'532819','MPHASIS':'526299','HEXAWARE':'532861','NIIT':'500304','KPIT':'542651','LTTS':'540115','COFORGE':'532541','ZENSAR':'504067','RAMSYSTEMS':'532370','MASTEK':'523704','SASKEN':'532663','TATAELXSI':'500408','CYIENT':'532175','SONATSOFTW':'532221','TANLA':'532790','LTIM':'540005','INFY':Kc,'ROUTE':'543228','BSOFT':'526301','NEWGEN':'540900','INTELLECT':'538835','NUCLEUS':'531209','NELCO':'504112','DELTACORP':'532840','WONDERLA':'538268','MAHINDCIE':'532756','STARHLTH':'543412','NAUKRI':'532777','JUSTDIAL':'535648','MATRIMONY':'539846','MAKEMYTRIP':Kk,'IXIGO':'544229','RATEGAIN':'543417','TEAMLEASE':'539658','QUESS':'539978','SIS':'540673','SECURKLOUD':'539963','HAPPYFORGE':'543532','KALYANKJIL':'543278','SENCO':'543456','THANGAMAYL':'531509','TRIBHOVAND':'512415','PC JEWELLER':'534809','RAJESHEXPO':'531500'}
	@A.cache_data(ttl=600)
...
try:
	...
	if Q(CI)>0:
		O8,O9,OA,OB,OC,OD,OE=A.tabs(['🚨 Latest Alerts Timeline','🏢 Alerts by Stock','📰 Smart News Engine (1 Day)','📰 Smart News Engine (All News)','📢 Corporate Announcements','📢 DOCUMENTS HUB','📜 Rules']);Cw=[];It=CI[:30]
		with A.spinner('Scanning Top 30 stocks for Circuit & 52-Week Breakouts (15 Days)...'):
			for F in It:
				A1=G(F).strip();BB=O2(A1,limit=15)
				for Aw in BB:Aw[AZ]=A1;Cw.append(Aw)
		OF={A[AZ]for A in Cw};Iu={A[AZ]for A in Cw if Ab in A[L]or Ac in A[L]or Ad in A[L]or AH in A[L]}
		def OG(sym):
			A=sym
			if A in Iu:B,C,D=Aa,'#003300','#0fbf62'
		...
		with O8:
			OH,OI,OJ=A.columns([2,1,1]);Iv=OH.text_input('🔍 Search Alerts:',placeholder='e.g. ICICIBANK, circuit...',key='global_news_search');Iw=OI.selectbox('⏳ Time Filter:',['All (Up to 15 Days)',Ko,Kp],key='global_news_time');OK=OJ.radio('↕️ Sort By Time:',[Kq,'Oldest First'],horizontal=B,key='global_news_sort');Ax=Cw.copy()
			if Iv:Ix=Iv.lower();Ax=[A for A in Ax if Ix in A[AZ].lower()or Ix in A[KW].lower()]
			if Iw==Kp:Ax=[A for A in Ax if Ab in A[L]or Ac in A[L]or Ad in A[L]or AH in A[L]]
			elif Iw==Ko:OL=H.Timestamp.now(tz=BM);Ax=[A for A in Ax if(OL-A[AI]).total_seconds()/86400<=7.]
			Ax.sort(key=lambda x:x[AI],reverse=OK==Kq);A.markdown(CX,unsafe_allow_html=B)
			if Ax:
		...
		with O9:
			ON=A.columns(2);Fc=0
			for A1 in[G(A).strip()for A in It]:
		...
		with OA:
			...
			for Cx in CI[:10]:
				A1=G(Cx).strip();BB=O3(A1,limit=5)
				if BB:
					with OQ[Fe%2]:
		...
		with OB:
			...
			for Cx in CI[:10]:
				A1=G(Cx).strip();BB=O4(A1,limit=6)
				if BB:
					with OR[Ff%2]:
		...
		with OC:
			...
			for Cx in CI[:15]:
				A1=G(Cx).strip();Fi=O5(A1,limit=7)
				if Fi:
					with OT[Fh%2]:
		...
		with OD:
			...
			with OV:Iy=[G(A).strip()for A in CI[:60]];Iz=A.multiselect('🔍 Stocks to view:',options=Iy,default=Iy[:4],key='doc_hub_stocks_v2')
			with OW:OY=A.selectbox('📅 Date range:',[Gh,Kr,Ks,Gi],index=1,key='doc_days_v2')
			with OX:I_=A.selectbox('📋 Rows per section:',[3,5,8,12],index=1,key='doc_limit_v2')
			OZ={Gh:30,Kr:90,Ks:180,Gi:365};Oa=OZ[OY]
			if not Iz:A.info('Select at least one stock above to view its documents.')
				...
				for o in Iz:
					...
					with A.expander(f"📁  {o}   {"· BSE "+v if v else"· BSE code not mapped — Screener links shown"}",expanded=B):
						Fk="<div style='display:flex; flex-wrap:wrap; gap:8px; margin-bottom:14px;'>";Ob=[('📢 BSE Announcements',f"https://www.bseindia.com/corporates/Corp_Annoucement.html?expandable=0&scripcd={v}"if v else f"https://www.nseindia.com/companies-listing/corporate-filings-announcements?symbol={o}",Kt,Ku),('📑 Annual Reports',f"https://www.screener.in/company/{o}/",CW,Kv),('⭐ Credit Ratings',f"https://www.screener.in/company/{o}/",GA,'#f57f17'),('🎙️ Concalls',f"https://www.screener.in/company/{o}/",'#fce4ec',DI),('📊 Investor PPT',f"https://www.bseindia.com/corporates/Inv_Rel.aspx?scripcd={v}"if v else f"https://www.screener.in/company/{o}/",Kw,Kx),('🏛️ NSE Filings',f"https://www.nseindia.com/companies-listing/corporate-filings-announcements?symbol={o}",'#e0f7fa','#00695c'),('📈 Screener',f"https://www.screener.in/company/{o}/",'#fffde7',A_)]
						...
						with Oc:
							A.markdown("<p style='font-weight:700; font-size:0.9em; border-bottom:2px solid #5c6bc0; padding-bottom:4px; color:#5c6bc0;'>📢 Announcements</p>",unsafe_allow_html=B);J0=CJ.get(Gj,[])
							if J0:
								Og,Oh=A.tabs([GM,'All ↗'])
								with Og:
									for CK in J0[:I_]:BC=CK[A5][:85]+'…'if Q(CK[A5])>85 else CK[A5];Bh=f"<a href='{CK[f]}' target='_blank' style='color:#5c6bc0; text-decoration:none;'>{BC}</a>"if CK[f]else f"<span>{BC}</span>";A.markdown(f"<div style='font-size:0.82em; margin-bottom:6px; border-left:3px solid #c5cae9; padding-left:6px;'>{Bh}<br><span style='color:#aaa; font-size:0.85em;'>{CK[BJ]}</span></div>",unsafe_allow_html=B)
						...
						with Od:
							A.markdown("<p style='font-weight:700; font-size:0.9em; border-bottom:2px solid #43a047; padding-bottom:4px; color:#43a047;'>📑 Annual Reports</p>",unsafe_allow_html=B);J1=CJ.get(Gk,[])
							if J1:
								for Dr in J1[:6]:J2=Dr[BJ][:4]if Dr[BJ]else'Report';Bh=f"<a href='{Dr[f]}' target='_blank' style='color:#43a047; text-decoration:none;'>📄 Annual Report {J2}</a>"if Dr[f]else f"<span>📄 Annual Report {J2}</span>";A.markdown(f"<div style='font-size:0.82em; margin-bottom:5px;'>{Bh}</div>",unsafe_allow_html=B)
							else:
						...
						with Oe:
							A.markdown("<p style='font-weight:700; font-size:0.9em; border-bottom:2px solid #f57f17; padding-bottom:4px; color:#f57f17;'>⭐ Credit Ratings</p>",unsafe_allow_html=B);J3=CJ.get(Gl,[])
							if J3:
								for CL in J3[:4]:BC=CL[A5][:70]+'…'if Q(CL[A5])>70 else CL[A5];Bh=f"<a href='{CL[f]}' target='_blank' style='color:#f57f17; text-decoration:none;'>{BC}</a>"if CL[f]else f"<span>{BC}</span>";A.markdown(f"<div style='font-size:0.82em; margin-bottom:5px; border-left:3px solid #ffe0b2; padding-left:6px;'>{Bh}<br><span style='color:#aaa; font-size:0.85em;'>{CL[BJ]}</span></div>",unsafe_allow_html=B)
						...
						with Of:
							A.markdown("<p style='font-weight:700; font-size:0.9em; border-bottom:2px solid #e53935; padding-bottom:4px; color:#e53935;'>🎙️ Concalls &amp; Investor Docs</p>",unsafe_allow_html=B);Ok=CJ.get(Gm,[]);J4=CJ.get(ET,[]);J5=J4+Ok
							if J5:
								for Bi in J5[:I_]:Ol=Bi in J4;J6='📊'if Ol else'🎙️';BC=Bi[A5][:70]+'…'if Q(Bi[A5])>70 else Bi[A5];Bh=f"<a href='{Bi[f]}' target='_blank' style='color:#e53935; text-decoration:none;'>{J6} {BC}</a>"if Bi[f]else f"<span>{J6} {BC}</span>";A.markdown(f"<div style='font-size:0.82em; margin-bottom:5px; border-left:3px solid #ffcdd2; padding-left:6px;'>{Bh}<br><span style='color:#aaa; font-size:0.85em;'>{Bi[BJ]}</span></div>",unsafe_allow_html=B)
							...
							A.markdown(CX,unsafe_allow_html=B);Fp="<div style='display:flex; gap:6px; flex-wrap:wrap;'>";Om=[('📝 Transcript',f"https://www.screener.in/company/{o}/",Kt,Ku),('🤖 AI Summary',f"https://www.screener.in/company/{o}/",CW,Kv),('📊 PPT',f"https://www.bseindia.com/corporates/Inv_Rel.aspx?scripcd={v}"if v else f"https://www.screener.in/company/{o}/",Kw,Kx),('▶️ REC',f"https://www.youtube.com/results?search_query={o}+concall+earnings",Bm,DA)]
