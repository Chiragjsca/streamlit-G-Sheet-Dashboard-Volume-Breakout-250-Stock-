...
# ==========================================
# 📉 EMA CROSSOVER SCREENER (EMA7 vs EMA11)
# ==========================================
import plotly.graph_objects as go
from datetime import timedelta

st.markdown("<p style='font-size:0.85rem; font-weight:bold; margin:0; padding:0;'>📉 Stock EMA Crossover Analysis</p>", unsafe_allow_html=True)
st.caption("Scans tickers for EMA7 / EMA11 crossovers in the last 7 trading days — dark-themed candlestick chart with range slider.")

with st.expander("📉 Click to view EMA7 / EMA11 Crossover Screener", expanded=False):
    ...
    ema_end_date = datetime.now()
    ema_start_date = ema_end_date - timedelta(days=120)

    def analyze_ema_ticker(ticker):
        try:
            stock_data = yf.download(
                ticker, start=ema_start_date, end=ema_end_date,
                interval='1d', auto_adjust=True, progress=False
            )
        except Exception:
            return None, None

        if stock_data is None or stock_data.empty or len(stock_data) < 12:
            return None, None
        ...
        stock_data['EMA11'] = stock_data['Close'].ewm(span=11, adjust=False).mean()
        stock_data['Crossover'] = stock_data['EMA7'] > stock_data['EMA11']

        last_7_days = stock_data.tail(7)
        if len(last_7_days) < 2:
            return None, None

        if last_7_days['Crossover'].iloc[-1] and not last_7_days['Crossover'].iloc[-2]:
            return stock_data, "EMA7 crossed above EMA11"
        elif not last_7_days['Crossover'].iloc[-1] and last_7_days['Crossover'].iloc[-2]:
            return stock_data, "EMA7 crossed below EMA11"
        else:
            return None, None

    if st.button("🔍 Run EMA Crossover Scan", key="ema_crossover_scan_btn"):
        with st.spinner("Scanning tickers for EMA7 / EMA11 crossovers..."):
            found_any = False
            for ema_ticker in ema_tickers:
                ema_stock_data, ema_crossover_result = analyze_ema_ticker(ema_ticker)
                if ema_crossover_result:
                    found_any = True
                    st.subheader(f"{ema_ticker}: {ema_crossover_result}")

                    ema_fig = go.Figure()
                    ema_fig.add_trace(go.Candlestick(
                        x=ema_stock_data.index,
                        open=ema_stock_data['Open'],
                        high=ema_stock_data['High'],
                        low=ema_stock_data['Low'],
                        close=ema_stock_data['Close'],
                        name='Candlestick'
                    ))
                    ema_fig.add_trace(go.Scatter(
                        x=ema_stock_data.index, y=ema_stock_data['EMA7'],
                        name='EMA 7', line=dict(color='green')
                    ))
                    ema_fig.add_trace(go.Scatter(
                        x=ema_stock_data.index, y=ema_stock_data['EMA11'],
                        name='EMA 11', line=dict(color='orange')
                    ))
                    ema_fig.update_layout(
                        title=f'{ema_ticker} Price and EMA Crossover',
            ...
            if not found_any:
                st.info("No EMA7/EMA11 crossovers detected in the last 7 days for the tracked tickers.")
