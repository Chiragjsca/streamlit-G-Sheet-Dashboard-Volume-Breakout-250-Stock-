...
if not raw_df.empty:
    ...
    with st.expander(f"🚀 Executive Dashboard — {selected_sheet}", expanded=True):
        ...
        # ---------- Chart row 5: Top 20 Below 200 DMA / Top 20 Above 200 DMA ----------
        dash_n3, dash_n4 = st.columns(2)

        with dash_n3:
            if diff200_series.notna().any():
                below_idx = diff200_series[diff200_series < 0].dropna().sort_values(ascending=True).head(20).index
                ...
                if not below_d.empty:
                    ...
                    fig_below200.update_layout(title="📉 Top 20 Below 200 DMA", template="plotly_white", height=560, margin=dict(t=40, b=10, l=10, r=10))
                    st.plotly_chart(fig_below200, use_container_width=True, key=f"dash_below200_{selected_sheet}", config=DASH_CHART_CONFIG)
                else:
        ...
        with dash_n4:
            if diff200_series.notna().any():
                above_idx = diff200_series[diff200_series > 0].dropna().sort_values(ascending=False).head(20).index
                ...
                if not above_d.empty:
                    ...
                    fig_above200.update_layout(title="🎯 Top 20 Above 200 DMA", template="plotly_white", height=560, margin=dict(t=40, b=10, l=10, r=10))
                    st.plotly_chart(fig_above200, use_container_width=True, key=f"dash_above200_{selected_sheet}", config=DASH_CHART_CONFIG)
                else:
