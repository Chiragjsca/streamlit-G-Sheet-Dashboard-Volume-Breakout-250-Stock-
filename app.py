...
if not raw_df.empty:
    ...
    # ==========================================
    # 🏆 MULTI-HORIZON PERFORMANCE SUMMARY TABLE OVERHAUL
    # ==========================================
    st.markdown("---")
    st.markdown("### 📈 Multi-Horizon Performance Summary Matrix")
    
    # 27 Distinct lookback definitions matching column configurations
    horizons = [
    ...
    # Header Tools layout tracking your template screenshot style
    ...
    # Collect and map existing horizon data metrics dynamically from column listing
    detected_metric_map = {}
    
    # Map default tracking metrics
    ...
    if detected_metric_map:
        ...
        for idx, row in filtered_df.iterrows():
            ...
            # Map metrics values safely converting strings to floats dynamically
        ...
        # Filter performance dataset based on local search layout text box input
        if summary_search:
            perf_df = perf_df[perf_df["STOCK NAME"].str.contains(summary_search, case=False, na=False)]
            
        # Execute sorting strategy transformation safely 
        ...
        # Inject dynamic ranking calculation
        perf_df.insert(0, "RANK", perf_df.index + 1)
        
        # Build AgGrid for the Performance Summary
        perf_gb = GridOptionsBuilder.from_dataframe(perf_df)
        perf_gb.configure_column("RANK", width=80, pinned="left", type=["numericColumn"])
        perf_gb.configure_column("STOCK NAME", width=140, pinned="left")
        perf_gb.configure_column("CURRENT PRICE", width=130, type=["numericColumn"])
        
        # Conditional format rule code engine rendering positive values as green text and negative as red text
        color_code_js = JsCode("""
        function(params) {
            if (params.value === undefined || params.value === null) return null;
            let val = parseFloat(params.value);
            if (val > 0) {
                return { 'color': '#0f9d58', 'fontWeight': 'bold' };
            } else if (val < 0) {
                return { 'color': '#ea4335', 'fontWeight': 'bold' };
            }
            return null;
        }
        """)
        
        # Format columns with custom renderer displaying the '%' symbol back onto float values
        ...
        for h in detected_metric_map.keys():
            if h in perf_df.columns:
                perf_gb.configure_column(
                    h, 
                    width=120, 
                    type=["numericColumn"], 
                    cellStyle=color_code_js,
                    cellRenderer=percent_formatter_js
                )
                
        perf_gb.configure_grid_options(
            domLayout="normal",
            rowHeight=38,
            headerHeight=45,
        ...
        AgGrid(
            perf_df,
            gridOptions=perf_grid_ops,
            theme="streamlit",
            allow_unsafe_jscode=True,
            fit_columns_on_grid_load=False,
            height=450,
            width='100%',
            key="horizon_perf_grid"
        )
    else:
        st.warning("No percentage performance lookback columns (like Price %, 1 Week %, etc.) detected inside this Google Sheet tab dataset framework.")
