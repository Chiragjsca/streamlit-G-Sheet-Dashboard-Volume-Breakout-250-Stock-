...
"""
            ...
            with ws_tabs[12]:
                ...
                st.caption("📡 News loads on tap, so switching between stocks stays fast.")
                if st.button(f"📰 Load Latest News for {sym}", key=f"news_load_btn_{sym}", use_container_width=True):
                    with st.spinner(f"Fetching today's latest news for {sym}..."):
                        st.session_state["last_news_result"] = {"sym": sym, "news": fetch_single_stock_news(sym, limit=10)}

                _last_news = st.session_state.get("last_news_result", {})
                if _last_news.get("sym") == sym:
                    stock_news = _last_news.get("news", [])
                    if stock_news:
                    ...
                    st.info(f"Tap **Load Latest News** above to fetch news for {sym}.")
